classdef FaulhaberMCDC_VOLTMOD < cutter & loghandler
    % Faulhaber motor controller. Before use, set up your controller with the 
    % motor you have and the encoder. 
    %
    % BEFORE RUNNING:
    % Run the VOLTMOD command if you have no encoder. Save the settings to the 
    ...
    properties 
        ...
        maxControlValue=8280; %TODO: find the max control value. e.g. when motor 
    ...
    methods
        ...
        % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        %constructor
        function obj = FaulhaberMCDC_VOLTMOD(serialComms,logObject)
        ...
        function success = startVibrate(obj,cyclesPerSec)
            ...
            success = obj.sendReceiveSerial(sprintf('U-%d',commandedSpeed));
        end %startVibrate


        function success = stopVibrate(obj)
            success = obj.sendReceiveSerial('U-1380'); %10Hz
            pause(0.4)
            success = obj.sendReceiveSerial('U0');
            obj.disable;
        end %stopVibrate
